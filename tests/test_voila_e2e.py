...
def test_mask_too_small_shows_error_banner(voila_page, tmp_path) -> None:
    """Uploading a < 50 mm × 50 mm measured file must show a MASK_TOO_SMALL error banner."""
    import numpy as np
    import pandas as pd

    _log(">> test_mask_too_small_shows_error_banner")

    # Generate a 15 mm × 15 mm Gaussian SAR grid — smaller than the 50 mm inscribed-square threshold.
    step = 0.001
    ...
    assert "50 mm" in body_text, "Expected '50 mm' in MASK_TOO_SMALL error text"
